...
SCE(short-Circuit Evaluation)
	하나의 조건검사만으로 결과를 알수 있는 경우 
	Java 컴파일러는 뒤의 조건을 수행하지 않고 건너뛰게 된다.
...
*/
		...
		/*
			논리AND의 경우 둘중 하나만 false이면 false를 반환하므로,
			첫번째 조건이 false이면 두번째 조건은 검사(실행)하지 않는다.
		*/
		result = (num1+=10)<0 && (num2+=10)>0;
		//result = 10<0 (false) false가 출력
		System.out.println("result = "+result);
//		num2는 초깃값인 0을 출력한다
		System.out.println("num1 = "+num1+", num2 = "+num2);
		
		/*
			논리OR의 경우 둘중 하나만 true를 반환하므로
			첫번째 조건이 true이면 두번째 조건은 검사하지 않는다.
		*/
		result = (num1+=10)>0 || (num2+=10)>0;
//		result = 20>0 (true) true가 출력됨
		System.out.println("result = "+result);
//		num2은 여기서도 증가되지 않음
		System.out.println("num1 = "+num1+", num2 = "+num2);
