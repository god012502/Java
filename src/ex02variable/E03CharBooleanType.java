...
/*
boolean 타입 
 - true 혹은 false 두가지의 값만 가질수 있는 자료형으로
   산술연산(+,-), 비교연산(<, <=)에서는 사용할 수 없고, 논리연산에서만 사용할 수 있다
   
   && : 논리And로 양쪽 모두 true일 때만 true를 반환
   || : 논리Or로 둘 중 하나만 true여도 true를 반환
*/
	public static void main(String[] args) {
/*
char형 
	하나의 문자를 저장할 수 있는 자료형으로 값을 저장할 때 '(Single quotation)으로 감싸서 표현한다.
...
유니코드
	1byte로 표현할 수 있는 문자로 영어, 숫자를 제외한 모든 문자를 
...
//		char형으로 강제 형변환 후 출력 67 => C
		char ch3 = (char)(ch2+num1);
		System.out.println("ch3(문자출력)=> "+ch3);

//		int형으로 강제 형변환 후 출력 C => 67
		System.out.println("ch3(아스키코드출력)=>"+(int)ch3);
		
//		싱글쿼테이션으로 감쌋으므로 문자1 (아스키코드 : 49)
		char ch4 = '1';
//		연산결과는 49+1 = 50이므로 이에 해당하는 문자는 '2'
		char ch4_1 = '1'+1;
//		문자 '2'와 정수로 강제 형변환한 50이 출력됨
...
//		문자 '가'는 유니코드 44032로 표현되므로  true로 반환
		bn3 = '가' > 30000;
