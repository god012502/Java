...
/*
상수 
 저장된 값이 변하지 않는 메모리의 한 종류로 변수를 선언한 후 final을 추가하면 된다.
 - 상수는 한번만 초기화되고, 그 이후 값을 변경할 수 없다.
 - 선언 시 전체를 대문자로 기술하고, 연결되는 단어가 있다면 언더바를 사용한다.
...
//		가위, 바위, 보 게임 제작을 위해 개발자가 1,2,3으로 정한 후 코딩한다고 가정하면..
//	    상수 선언 및 초기화
	    final int SCISSOR = 1;
	    final int ROCK = 2;
	    final int PAPER = 3;
...
//	    1과 3은 개발자 본인만 알 수 있는 내용이므로, 다른 사람이 읽었을 때는 가독성이 떨어진다. 
	    computer = 1;
	    user = 3;
