...
/*
 정수 자료형
  ...
  - 일반적으로 CPU(중앙처리장치)는 정수형 연산을 가장 빠르게 처리하는 특징이 있다.
    즉 int에 최적화 되어있다.
  - 따라서, 정수형 변수를 사용할 때에는 대부분 int형을 사용하면 된다. 
 */
...
/*
 ...
 Java에서는 정수와 정수를 연산하면 정수의 결과가 반환된다.
 만약, 실수의 결과를 반환받고 싶다면 형변환(Type casting)을 해야 한다.
 */
		result1 = int1 / int2;
		System.out.println("int1 / int2 = "+result1);
...
이때, 접미사(L 혹은 l)을 붙여서 int형의 자료가 아닌 long형의 자료임을 표시해야 한다.
이와 같이 입력되는 숫자도 자료형을 기반으로 메모리에 로드되는데
