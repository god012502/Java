...
//		byte가 short보다 작은 자료형이므로 자동형변환 됨
		s1 = b1;
		System.out.printf("b1은 %d, s1은 %d%n", b1, s1);
		
/*
아래 코드는 자동형변환은 아니다.
CPU는 int보다 작은 자료를 연산할 때 int로 간주하여 진행하고 결과로 int로 반환한다.
따라서 이 부분은 int형에 최적화된 CPU의 특성으로 생각하면 된다. 
*/
		int num1 = b1+s1;
...
/*
명시적(강제) 형변환
큰 상자의 자료를 작은 상자의 자료형에 할당 해야 할 때 사용
...
/*
서식 문자 사용시 %.2f는 정수부는 모두 출력하고,
소수 이하는 2자리까지만 표현한다.
*/		
		System.out.printf("데이터손실 : num3 = %d, dl = %.2f %n", num3, dl);
		
/*
문자는 메모리에 저장 시 아스키코드로 저장되므로 int형과의 연산을 진행한 후 
문자로 표현하고 싶다면 char형으로 강제 형변환 하면 된다.
*/		
		char ch3 = 'A', ch4;
		int num4 = 2;
